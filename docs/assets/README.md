...
That command only applies where the raster is a direct Mermaid render of the
`.mmd`. It does not apply to `evidence-by-route.png`: that figure follows the
article house style and is generated by `make_route_evidence_figure.py` in the
articles repository, not from its `.mmd`, which records the semantic source
only.
