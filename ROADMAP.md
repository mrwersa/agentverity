...
As of 0.23.0, the released 0.23.0 picture is a complete local qualification
...
| Evaluation-period claim language | Evidence from one evaluation period can be mistaken for repeatability across deployments or time. Make the sampling unit and estimand visible at the point of use. | Text and machine reports distinguish the within-period disagreement probability from an across-period marginal claim and say when the evidence cannot support the latter | Evaluation-period identity is not yet a structured contract; caller provenance may be incomplete or sensitive | Improve conservative report guidance first; add structured fields only through the projection/provenance gate below |
