...
| Observed-count planning | After seeing flips, users can price only a fixed-rate projection or call a Python helper. Expose the audited fixed-count inverse without implying that its optimistic continuation is an early-admission rule. | `plan --observed FLIPS/PAIRS` reports the earliest all-agree total and, when supplied, whether a predeclared maximum remains reachable; examples reproduce the reviewed 1/73, 3/73, 4/73, and 8/73 boundaries | `best_case_admission_pairs` and its exact-boundary tests; a bare `--flips` option would omit evidence needed to validate the observation | Build as the next additive CLI capability; refuse malformed counts and keep suite planning behaviour unchanged |
