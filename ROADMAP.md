...
As of 0.23.1, the released 0.23.1 picture is a complete local qualification
...
An **evidence window** is a stored collection artefact. An **evaluation
period** is the statistical sampling unit used for a time-spanning claim; a
file boundary alone does not establish an independently sampled period.
...
| Two-budget method validation — delivered | The public validation exercised the 73-pair operating floor but not a larger fixed endpoint that can admit limited flips. Show how endpoint choice changes admission and curtailment savings without changing the rule. | Validation artefact v4 and its prose cover predeclared 73- and 146-pair endpoints; exact enumeration, exhaustive prefix states, and replay tests agree | The additional simulation cost is isolated in validation; no product evidence schema changed | Keep comparative efficiency claims tied to the validated IID within-period design; do not infer across-period validity from a larger budget |
...
| Retrospective curtailment replay — delivered in 0.23.0 | Teams with ordered evidence cannot see how much a predeclared impossibility rule would have saved. Replay the fixed-endpoint rule without rewriting the historical qualification. | `assess --replay-curtailment` reports the first unreachable pair and avoided pairs; the original endpoint call remains visible; versioned live replay fixtures match path by path | Ordered raw observations, one evidence-derived endpoint, and no recorded errors; global historical timing is not retained | Always label the result post-hoc counterfactual; expose no user-selected endpoint; never create an early class or claim that replay was a predeclared procedure |
| Evaluation-period claim language — delivered in 0.23.1 | Evidence from one evaluation period can be mistaken for repeatability across deployments or time. Make the sampling unit and estimand visible at the point of use. | Terminal, JSON, and JUnit reports carry one shared caveat distinguishing the within-period disagreement probability from an across-period marginal claim; entry-point prose distinguishes evaluation periods from stored evidence windows | Evaluation-period identity is not yet a structured contract; caller provenance may be incomplete or sensitive | Keep the conservative caveat on existing report surfaces; add structured identity fields only through the projection/provenance gate below |
...
The versioned method-validation v5 artefact names the sampling unit with
matching prose: a `larger_within_period_budget_is_not_cross_time_evidence`
machine key and an evaluation-period assumption. Rename artefact keys and
prose together in one reviewed revision; do not update either side piecemeal.
...
| Item | User problem and intended outcome | Success metric | Dependencies and risks | Build/defer gate |
|---|---|---|---|---|
| Across-period qualification research | A within-period result does not establish repeatability across time, model updates, or deployments. Determine whether repeated independent evaluation periods support a useful marginal claim without pretending they repair dependence inside a period. | An ADR defines the across-period marginal estimand, period sampling unit, aggregation rule, and error guarantee; simulation covers unequal periods, drift, and clustered trials; an independent statistician reviews the result | Multiple genuinely independent periods and stable projection identity; simply multiplying a one-period budget has no across-period guarantee | Do not add `--periods`, a report class, or schema fields until the guarantee and affordable budget are demonstrated; document and defer if they are not |
